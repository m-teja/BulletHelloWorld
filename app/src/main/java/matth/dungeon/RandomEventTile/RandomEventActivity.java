...
public class RandomEventActivity extends AppCompatActivity {
    ...
    private void getEvent(PlayerInfoPassUtility playerInfoPassUtility) {
        ...
        try {
            randomEvent = (RandomEvent)event.getDeclaredConstructor(PlayerInfoPassUtility.class, Context.class).newInstance(playerInfoPassUtility, this);
        }
        catch (Exception e) {
            e.printStackTrace();
        }
